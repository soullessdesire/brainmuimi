...
import { useAuth, useAllDocumentRequests, useUpdateRequestStatus, useDocuments } from '../../hooks'
...
type SidebarTab = 'requests' | 'documents'
  ...
  return (
    <div
      ...
      className={cn(
        ...
        active ? 'bg-accent/20 text-accent' : 'text-white/40 hover:text-white/75 hover:bg-white/5',
      )}
    >
      {icon}{label}
    </div>
...
function RequestsPanel() {
  ...
  async function handleAction(requestId: string, status: RequestStatus) {
    ...
    try {
      ...
      const label = status === 'approved' ? 'Access approved' : status === 'rejected' ? 'Access rejected' : 'Request revoked'
      toast.success(label)
    } catch {
  ...
  const filtered = requests.filter(r => {
    ...
    const matchSearch = !q || r.userName.toLowerCase().includes(q) || r.userEmail.toLowerCase().includes(q) || r.paymentRef.toLowerCase().includes(q) || getDocTitle(r.docId).toLowerCase().includes(q)
    ...
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      <div className="bg-card border-b border-border px-8 py-5">
        <div className="flex items-start justify-between gap-6 flex-wrap">
          <div>
            <h1 style={{ fontFamily: 'var(--font-display)' }} className="text-2xl font-bold tracking-tight mb-1">
              Document Access Requests
            </h1>
            <p className="text-xs text-muted-foreground">Verify payment references and approve per-document access</p>
          </div>
          <div className="flex items-center gap-5">
            {([
              ...
              <div key={label} className="text-center min-w-[48px]">
                <p style={{ fontFamily: 'var(--font-display)' }} className={cn('text-3xl font-bold leading-none mb-1', cls)}>{count}</p>
                <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-semibold">{label}</p>
              </div>
            ))}
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => void refetch()} disabled={isFetching}>
      ...
      <div className="bg-card border-b border-border px-8 py-3 flex items-center justify-between gap-4 flex-wrap">
          ...
          <TabsList className="h-8">
            {(['all', 'pending', 'approved', 'rejected'] as RequestTab[]).map(t => (
              <TabsTrigger key={t} value={t} className="text-xs capitalize gap-1.5">
                {t}
                <span className="bg-muted rounded-full px-1.5 py-0.5 text-[10px] font-semibold">{counts[t]}</span>
              </TabsTrigger>
            ))}
          </TabsList>
        </Tabs>
        <div className="relative">
          ...
          <Input placeholder="Search user, ref or document…" value={search} onChange={e => setSearch(e.target.value)} className="pl-8 h-8 text-xs w-72" />
        </div>
      </div>
                ...
                {filtered.map(req => (
                  <React.Fragment key={req.id}>
                    ...
                    >
                      ...
                      <TableCell>
                        <p className="text-sm font-medium max-w-[180px] truncate">{getDocTitle(req.docId)}</p>
                        <p className="text-xs text-accent font-semibold">{getDocPrice(req.docId)}</p>
                      </TableCell>
                      <TableCell>
                        <code className="text-xs font-mono bg-muted px-2 py-1 rounded">{req.paymentRef || '—'}</code>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell><Badge variant={"ghost"}>{req.status}</Badge></TableCell>
                      <TableCell onClick={e => e.stopPropagation()}>
                        <div className="flex items-center gap-2">
                          {actingId === req.id ? <Spinner size={14} className="text-muted-foreground" /> : (
                            <>
                              {req.status !== 'approved' && (
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button size="sm" className="bg-emerald-700 hover:bg-emerald-800 text-white h-7 px-2.5 text-xs gap-1"
                              ...
                              {req.status !== 'rejected' && (
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button size="sm" variant="destructive" className="h-7 px-2.5 text-xs gap-1"
      ...
      <div className="bg-card border-b border-border px-8 py-5">
        <div className="flex items-start justify-between gap-6">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{documents.length} document{documents.length !== 1 ? 's' : ''} in the library</p>
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6">
        {isLoading ? (
          ...
          <div className="text-center py-24 flex flex-col items-center gap-4">
            ...
            <p style={{ fontFamily: 'var(--font-display)' }} className="text-xl text-foreground">No documents yet</p>
      ...
      <div className="flex min-h-screen bg-background">
        <aside className="hidden lg:flex w-56 shrink-0 flex-col bg-foreground px-4 py-7 gap-6">
          ...
          <nav className="flex flex-col gap-1 flex-1">
            <NavItem icon={<FileText size={15} />} label="Requests"  active={sidebar === 'requests'}  onClick={() => setSidebar('requests')} />
            <NavItem icon={<Users size={15} />}    label="Documents" active={sidebar === 'documents'} onClick={() => setSidebar('documents')} />
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          {sidebar === 'requests' ? <RequestsPanel /> : <DocumentsPanel />}
        </main>
      </div>
    </TooltipProvider>
  )
